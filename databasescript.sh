...
echo "#############################################################################################"
echo "BU,SQLVM, resourceGroup, region, LicenseType,SKUImageOffer, SKUImageSKU,Zones, ResourceID"
az graph query --first 1000 -q "resources \
| where type contains 'microsoft.sqlvirtualmachine/sqlvirtualmachines' \
| extend same = tostring(name) \
| join kind = leftouter ( resources \
| where type == 'microsoft.compute/virtualmachines' \
| extend same = tostring(name) ) on same \
...
| sed 's/\t/,/g' | change_name \
| sed 's/AHB/AzureHybridBenefit/g' \
