...
az graph query -q "resources| where subscriptionId =='$sid' | project name, resourceGroup, location, zones, properties.sku.tier, properties.sku.capacity,properties.sku.family, properties.operationalState, id" -o tsv \
 --query "data[].{name:name, resourceGroup:resourceGroup, Region:location, zonesEnabled:zones, sku_tier:properties_sku_tier, sku_capacity:properties_sku_capacity, generation:properties_sku_family, properties_operationalState:properties_operationalState, ResourceId:id}" \
| sed 's/\t/,/g' \
| sed 's|97849348-0525-4007-9d79-11ada732dfcc|'SG01'|g' \
| sed 's|d0259ea3-0613-49b4-8bfb-f1c930e15b61|'AH01'|g' \
| sed 's|412b59ce-fbe6-4ecd-88ac-053c127481b6|'AU01'|g' \
