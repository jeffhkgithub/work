...
do
    ...
    az graph query -q "resources| where subscriptionId =='$sid' | wshere type == 'microsoft.apimanagement/service' | project name, resourceGroup, location, zones, sku.name, sku.capacity, id" -o tsv --query "data[].{name:name, resourceGroup:resourceGroup, location:location, zones:zones, sku_name:sku_name, sku_capacity:sku_capacity,  ResourceId:id}" | sed 's/\t/,/g' | sed 's|^|'$BUname,'|g'
    echo "#############################################################################################"
